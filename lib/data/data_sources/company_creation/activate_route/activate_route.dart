...
class ActivateRoute {
  ...
  ActivateRoute(this.httpClient, this.runSafely, this.sharedPreferences);

  ResultFuture<RouteResponse> call({required String routeId,}) {
    return runSafely(
      () async {
        final response = await httpClient.post(
          "${Urls.activateRoute1}$routeId${Urls.activateRoute2}",
        );

        if (response.isOk) {
          final data = RouteResponse.fromJson(response.data);
          Logger.logInfo("Route Activation response : ${data.toJson()}");
          return data;
        }
