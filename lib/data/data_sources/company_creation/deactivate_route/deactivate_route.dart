...
class DeactivateRoute {
  ...
  DeactivateRoute(this.httpClient, this.runSafely, this.sharedPreferences);

  ResultFuture<RouteResponse> call({required String routeId,}) {
    return runSafely(
      () async {
        final response = await httpClient.post(
          "${Urls.deactivateRoute1}$routeId${Urls.deactivateRoute2}",
        );

        if (response.isOk) {
          final data = RouteResponse.fromJson(response.data);
          Logger.logInfo("Route Dectivation response : ${data.toJson()}");
          return data;
        }
