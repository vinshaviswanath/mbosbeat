...
class StateList {
  ...
  StateList(this.httpClient, this.runSafely, this.sharedPreferences);

  ResultFuture<StateListDtos> call(int countryId) {
    return runSafely(
      () async {
        final response = await httpClient.get("${Urls.stateList}$countryId");

        if (response.isOk) {
          final data = StateListDtos.fromJson(response.data);
          return data;
        }
